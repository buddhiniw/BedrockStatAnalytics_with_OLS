...
## @knitr check_data
###################################################################
# Check data file format and remove NA's
###################################################################
...
} else if ('SellingPrice' %ni% colnames(dataIn)[1]){ 
...
} else if(rowSums(is.na(dataIn))>1){
...
if(flagNotEnoughData || flagMissingPrice || flagPriceColNotFirst || flagLastRowNAData || flagMissingData){
